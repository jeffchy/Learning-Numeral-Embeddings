...
class Data:
    def __init__(self):
        # embedding mode should be in
        # ['token', 'GMM', 'prototype', 'LSTM', 'none']
        self.mode = 'none'
        ...
        self.dev_dir = None
        self.test_dir = None
        self.test_augment_dir = None
        self.test_harder_dir = None
        self.raw_dir = None
        ...
        self.test_texts = []
        self.raw_texts = []
        self.test_augment_texts = []
        self.test_harder_texts = []

        self.train_Ids = []
        self.dev_Ids = []
        self.test_Ids = []
        self.raw_Ids = []
        self.test_augment_Ids = []
        self.test_harder_Ids = []
    ...
    def show_data_summary(self):
        ...
        print("     Test Augment file directory: %s"%(self.test_augment_dir))
        print("     Test harder file directory: %s"%(self.test_harder_dir))
        ...
        print("     Test Augment instance number: %s"%(len(self.test_augment_texts)))
        print("     Test Harder instance number: %s"%(len(self.test_harder_texts)))
        print("     Raw   instance number: %s"%(len(self.raw_texts)))
        print("     Train Embeddings: %s"%(self.train_embedding))
        print("     Embedding Scheme: %s"%(self.mode))
        ...
        for line in in_lines:
            if len(line) > 2:
                ...
                ## if sequence labeling data format i.e. CoNLL 2003
                    ...
                    self.word_alphabet.add(word)
                    ## build feature alphabet
                    # we do not use feature
                    # uncomment to automaticly use features, pos and normalized number
    ...
    def build_pretrain_emb(self):
        ...
        if self.mode == 'none':
            ...
            if self.word_emb_dir:
                print("Load pretrained word embedding, norm: %s, dir: %s" % (self.norm_word_emb, self.word_emb_dir))
                self.pretrain_word_embedding, self.word_emb_dim = build_pretrain_embedding_fixed(
                                                                                           self.word_emb_dir,
                                                                                           self.preprocess_dir,
                                                                                           self.embed_epoch,
                                                                                           self.word_alphabet,
            ...
            if self.word_emb_dir:
                print("Load pretrained word embedding, norm: %s, dir: %s" % (self.norm_word_emb, self.word_emb_dir))
                self.pretrain_word_embedding, self.word_emb_dim = build_pretrain_embedding_prorotype(
                                                                                           self.word_emb_dir,
                                                                                           self.preprocess_dir,
                                                                                           self.embed_epoch,
                                                                                           self.alpha,
            ...
            if self.word_emb_dir:
                print("Load pretrained word embedding, norm: %s, dir: %s" % (self.norm_word_emb, self.word_emb_dir))
                self.pretrain_word_embedding, self.word_emb_dim = build_pretrain_embedding_token(
                                                                                           self.word_emb_dir,
                                                                                           self.preprocess_dir,
                                                                                           self.embed_epoch,
                                                                                           self.word_alphabet,
            ...
            if self.word_emb_dir:
                print("Load pretrained word embedding, norm: %s, dir: %s" % (self.norm_word_emb, self.word_emb_dir))
                self.pretrain_word_embedding, self.word_emb_dim = build_pretrain_embedding_LSTM(
                                                                                           self.word_emb_dir,
                                                                                           self.preprocess_dir,
                                                                                           self.embed_epoch,
                                                                                           self.word_alphabet,
            ...
            if self.word_emb_dir:
                print("Load pretrained word embedding, norm: %s, dir: %s" % (self.norm_word_emb, self.word_emb_dir))
                self.pretrain_word_embedding, self.word_emb_dim = build_pretrain_embedding_GMM(
                                                                                           self.word_emb_dir,
                                                                                           self.preprocess_dir,
                                                                                           self.embed_epoch,
                                                                                           self.word_alphabet,
    ...
    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, portion=self.train_portion)
        elif name == "dev":
            ...
            self.test_augment_texts, self.test_augment_Ids = read_instance(self.test_augment_dir, self.word_alphabet, self.char_alphabet,
            ...
            self.test_harder_texts, self.test_harder_Ids = read_instance(self.test_harder_dir, self.word_alphabet,
                                                                           self.char_alphabet,
                                                                           self.feature_alphabets, self.label_alphabet,
    ...
    def write_decoded_results(self, predict_results, name):
        ...
        if name == 'raw':
           content_list = self.raw_texts
            ...
            content_list = self.test_augment_texts
        elif name == 'dev':
    ...
    def write_nbest_decoded_results(self, predict_results, pred_scores, name):
        ...
        if name == 'raw':
           content_list = self.raw_texts
            ...
            content_list = self.test_harder_texts
        elif name == 'dev':
            content_list = self.dev_texts
        elif name == 'train':
            content_list = self.train_texts
    ...
    def read_config(self,config_file):
        ...
        the_item = 'preprocess_dir'
        if the_item in config:
            self.preprocess_dir = config[the_item]


        the_item = 'train_embedding'
        if the_item in config:
            self.train_embedding = config[the_item]
    ...
    for line in fins:
        ...
        if "=" in line:
            ...
            if item=="feature":
                ...
                assert pair[-1] in ['GMM','prototype','LSTM','token','none']
                config[item] = pair[-1]
            elif item == 'train_embedding':
                config[item] = str2bool(pair[-1])
            else:
                if item in config:
