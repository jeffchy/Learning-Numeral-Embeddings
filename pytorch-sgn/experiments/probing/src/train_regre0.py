...
import argparse
from model import ListMax, CNN, TransformerModel, MLP, MLP1, MLP0, MLP00
import pickle
import numpy as np
import random
import logging
...
def train(args, logger, model_save_dir, val_dataset, test_dataset, train_dataset):
    ...
    if args.model == 'MLP':
        model = MLP00(args.hidden_dim, pretrain_embed)
    ...
    logger.info('TEST: epoch:{}-loss:{}'.format(epoch, test_loss))
    logger.info('BEST-DEV-LOSS: {}, BEST-DEV-TEST-LOSS:{}'.format(best_dev_loss, best_dev_test_loss))
    torch.save(best_dev_model, model_save_dir+'/model-{}-{}-{}.pt'.format(best_dev_test_loss, args.lr, args.hidden_dim))
